...
manifests: controller-gen crd-to-markdown ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	...
	$(CRD_MARKDOWN) -f apis/repo-manager.$(CR_DOMAIN)/v1beta3/pulp_types.go -n $(CR_KIND) > controllers/repo_manager/README.md
	$(CRD_MARKDOWN) -f apis/repo-manager.$(CR_DOMAIN)/v1beta3/pulp_backup_types.go -n $(CR_KIND)Backup > controllers/backup/README.md
	$(CRD_MARKDOWN) -f apis/repo-manager.$(CR_DOMAIN)/v1beta3/pulp_restore_types.go -n $(CR_KIND)Restore > controllers/restore/README.md
...
rename: ## Replace Custom Resource name
	...
	sed -i "s/default:=\"pulp\"/default:=\"${LOWER_CR_KIND}\"/" apis/repo-manager.pulpproject.org/v1beta3/pulp_types.go
	sed -i "s|quay.io/pulp/pulp-minimal|${APP_IMAGE}|g" apis/repo-manager.pulpproject.org/v1beta3/pulp_types.go
	sed -i "s|quay.io/pulp/pulp-web|${WEB_IMAGE}|g" apis/repo-manager.pulpproject.org/v1beta3/pulp_types.go
	...
	mv config/samples/repo-manager.pulpproject.org_v1beta3_pulp.yaml config/samples/repo-manager.${CR_DOMAIN}_v1beta3_pulp.yaml
