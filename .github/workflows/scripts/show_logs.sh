...
echo ::group::OBJECTS
sudo -E kubectl get pulp,pulpbackup,pulprestore,pvc,configmap,serviceaccount,secret,networkpolicy,ingress,service,deployment,statefulset,hpa,job,cronjob --all-namespaces -o yaml
echo ::endgroup::

sudo -E kubectl -n openshift-ingress get pvc,configmap,serviceaccount,secret,networkpolicy,ingress,route,service,deployment,statefulset,hpa,job,cronjob -o yaml
